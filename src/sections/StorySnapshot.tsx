...
type StorySnapshotResponse = Awaited<
  ReturnType<typeof client.queries.storySnapshot>
>

interface StorySnapshotVisualProps {
  response: StorySnapshotResponse
  sectionRef: React.RefObject<HTMLElement | null>
}

function StorySnapshotVisual({
  response,
  sectionRef,
  ...
  const badgeText =
    story?.badgeText ?? 'THE STORY'

  const title =
    story?.title ?? 'The Death of the Contract'

  const paragraphs =
    story?.paragraphs ?? [
      'In 2010, siblings Leo and Olga invested their life savings and professional expertise into rebuilding a distressed commercial property in Delray Beach, Florida. They entered into a lease-to-own agreement with a fixed purchase price and a four-year option period, intending to build a lasting family business through their restaurant, Eclectic Eats.',
      'After they say they exercised their contractual option to purchase the property, the dispute developed into prolonged litigation. Leo and Olga filed suit in Florida on October 30, 2014, seeking to enforce the agreement. Their crowdfunding campaign now seeks support for continued legal representation and pursuit of the claims described in the public record.',
    ]

  const buttonText =
    story?.buttonText ?? 'Read the full story'

  const image =
    story?.image ??
    '/photos/after-thumbs/after-0006.webp'

  const imageAlt =
    story?.imageAlt ??
    'Photograph of the restored commercial property and outdoor grounds in Delray Beach, Florida'
...
export default function StorySnapshot() {
  /*
   * IMPORTANT:
   * This hook must always be called on every render.
   * It must never be placed inside a conditional branch.
   */
  const sectionRef = useSectionReveal<HTMLElement>()

  const [storyResponse, setStoryResponse] =
    useState<StorySnapshotResponse | null>(null)

  useEffect(() => {
    let cancelled = false

    const loadStorySnapshot = async () => {
      try {
        const response =
          await client.queries.storySnapshot({
            relativePath: 'story-snapshot.json',
          })
        ...
        // Keep the existing content as the production fallback.
      }
    }
